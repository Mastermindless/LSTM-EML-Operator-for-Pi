"""Inference CLI for the LSTM-EML Pi controller — v02 (GPU-optimized).

Changes from v01:
    • Auto-detects MPS/CUDA/CPU device.
    • Uses cached_true_pi for faster reference comparison.
    • Batch inference mode (--batch-file) for throughput benchmarking.
...
from eml_operator import cached_true_pi, common_prefix_len, eml_pi
...
def main():
    ...
    if args.n is not None:
        run_once(model, args.n, use_length_input=use_length, device=device)
        return
    ...
    while True:
        ...
        if n < 1:
            ...
            continue
        run_once(model, n, use_length_input=use_length, device=device)
